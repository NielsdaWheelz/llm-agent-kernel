...
| K002 | Deterministic | `test_slice0_package.py::test_package_metadata_locks_qualified_git_dependencies` checks exact provider-runtime `69d41d38a3d290e7ae3bde9b57556dda41e1b2f1`, the immutable llm-tools revision, absence of retired private Codex packages, and the direct WebSocket client required by the shared App Server route without native-version admission. `test_slice0_contracts.py::test_dependency_provider_event_contracts_are_exact`, `test_dependency_web_search_operation_deadline_api_is_revisioned`, `test_dependency_web_read_revision_refreezes_exact_host_table_plan`, and `test_dependency_web_read_v2_extraction_locators_are_exact` exercise the exact public provider and Web APIs and identities. |
...
| K015 | Deterministic | `test_provider.py::test_resume_incompatibility_gets_exactly_one_cold_open`; `test_runtime_error_kinds_remain_distinct_and_close_the_session`; `test_kernel.py::test_restricted_projection_does_not_redispatch_an_uncertain_resume_failure`; `test_kernel.py::test_provider_stop_kinds_consume_without_automatic_retry` |
...
| K052 | Deterministic contract; shared-server live gate `NOT_RUN` | `test_slice0_package.py::test_production_never_calls_event_discarding_run_turn_projection`; opt-in `tests/live/test_codex_qualification.py::test_live_codex_stream_continuation_and_cancellation`; `test_live_synthetic_read_call_observation_and_close_reopen_resume`; `test_live_adversarial_shell_is_structured_or_contained_without_host_effect`; `test_live_structured_nested_optional_output`; `test_live_one_shot_uses_initial_read_before_first_provider_turn`; `test_live_json_encoded_tool_arguments`; `test_live_in_flight_cancellation`; `test_live_quota_exhaustion`. The probes require the declared external Unix-socket endpoint and observed server build and explicit group-shared cognition parent. Ordinary CI neither runs paid probes nor records private payloads. The shared-server gate requires explicit approval and is not satisfied by earlier private-runtime evidence. |
...
current shared App Server boundary. The 682-byte kernel base
...
application instructions, start/resume/reconstruction, continuing/isolated and
initial-Read paths, the retained custom-exec terminal rejection, fatal
`ProtocolDefect`, zero host dispatch/settlement, and inert observations.
An independent HTTPS fetch confirmed the provider commit, its parent, and the
annotated release tag; the focused corrected Codex suite passed 141 tests with
one skip. A broader dependency run on Darwin passed 1,014 tests and skipped two
but did not pass as a whole: two unrelated platform-sensitive checks failed on
`/proc/self/fd` availability and child-process-group reaping. Those failures are
...
with zero host effect, in-flight cancellation, restricted nested/nullable
structured output with commentary/final selection, isolated initial Read, and
JSON-string tool arguments. The suite used a temporary mode-700 qualification
...
Current paid-decision qualification: `test_decisions.py` proves original final
replay without opening a provider, unknown-before-poison parking, immutable
request identity, failed/substituted commit barriers, cold canonical continuation,
isolated replay without a repeated initial Read, stable scopes despite new run
IDs, and original Read positions across fresh claims. `test_kernel.py` covers
post-invocation timeout/cancellation/session uncertainty and preserved serial
host effects. These are kernel contract proofs; Jarvis's actual tables, admission,
role evidence, and BilledOnce recorder still require consumer integration gates.
